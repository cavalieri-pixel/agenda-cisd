// ACTUALIZACION FORZADA V2 - RUT AUTO Y BLOQUEO EDITABLE
...
export default function AppointmentModal({ isOpen, onClose, professionalId, startTime, onSuccess }) {
  ...
  const handleSearch = async () => {
    ...
    } catch { alert('Paciente no encontrado (se creará uno nuevo al agendar)'); }
  };
    ...
    try {
      if (activeTab === 'patient') {
        ...
        await axios.post(`${API_URL}/appointments`, {
          type: 'BLOCK',
          professionalId,
          startTime: new Date(blockStartTime),
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex gap-2">
                  <input className="border p-2 rounded w-full font-mono" value={rut} onChange={handleRutChange} placeholder="12.345.678-9" maxLength={12} required />
              ...
              <div>
                ...
                <select className="border p-2 rounded w-full" value={serviceCode} onChange={e => setServiceCode(e.target.value)}>{services.map(s => <option key={s.code} value={s.code}>{s.name} (${s.price})</option>)}</select>
              </div>
          ...
          {activeTab === 'block' && (
            <div className="space-y-4 bg-red-50 p-4 rounded-lg border border-red-100">
              ...
              <div><label className="text-xs font-bold text-gray-600">Motivo del Bloqueo</label><input className="border p-2 rounded w-full" value={blockTitle} onChange={e => setBlockTitle(e.target.value)} placeholder="Ej: Almuerzo, Congreso..." /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold text-gray-600">Bloquear Desde</label><input type="datetime-local" className="border p-2 rounded w-full bg-white" value={blockStartTime} onChange={e => setBlockStartTime(e.target.value)} required /></div>
                <div><label className="text-xs font-bold text-gray-600">Bloquear Hasta</label><input type="datetime-local" className="border p-2 rounded w-full bg-white" value={blockEndTime} onChange={e => setBlockEndTime(e.target.value)} required /></div>
              </div>
            </div>
          )}
