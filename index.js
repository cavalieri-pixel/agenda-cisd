...
// --- RUTA PÚBLICA: SLOTS (Auto-agendamiento) ---
...
// (Mantengo las otras rutas de pacientes igual, solo modifico la creación en la cita para ser más robusta)

// --- PROFESIONALES, SERVICIOS, HORARIOS (Sin cambios estructurales) ---
...
app.post('/api/professionals', async (req, res) => {
  ...
  try { const hp = await bcrypt.hash(password, 10); const n = await prisma.professional.create({ data: { name, email, password: hp, color, phone } }); res.json(n); } catch { res.status(500).json({ error: 'Err' }); }
});
app.put('/api/professionals/:id', async (req, res) => { const { name, email, color, phone } = req.body; try { const u = await prisma.professional.update({ where: { id: parseInt(req.params.id) }, data: { name, email, color, phone } }); res.json(u); } catch { res.status(500).json({ error: 'Err' }); } });
...
app.post('/api/professionals/import', async (req, res) => {
  const { data } = req.body; let c=0; for(const i of data) { try { const hp = await bcrypt.hash(i.password||'cisd123',10); await prisma.professional.create({ data: { name:i.name, email:i.email, password:hp, phone:i.phone||'', color:i.color||'#3788d8' } }); c++; } catch {} } res.json({ message: `Procesados: ${c}` });
});
...
app.post('/api/services', async (req, res) => {
  const { name, code, durationMin, price, isTelemed } = req.body; try { const n = await prisma.service.create({ data: { name, code, durationMin: parseInt(durationMin), price: parseInt(price), isTelemed } }); res.json(n); } catch { res.status(500).json({ error: 'Err' }); }
});
...
// *** RUTA CREAR CITA ACTUALIZADA CON DATOS COMPLETOS PACIENTE ***
app.post('/api/appointments', async (req, res) => {
  ...
  try {
    // 1. Upsert Paciente (Crear o Actualizar datos)
